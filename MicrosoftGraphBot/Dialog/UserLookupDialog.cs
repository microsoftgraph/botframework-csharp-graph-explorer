...
{
    [Serializable]
    public class UserLookupDialog : IDialog<User>
    {
        ...
        public async Task MessageReceivedAsync(IDialogContext context, IAwaitable<IMessageActivity> item)
        {
            ...
            PromptDialog.Text(context, async (IDialogContext textContext, IAwaitable<string> textResult) =>
            {
                ...
                else if (matches.Count == 1)
                {
                    ...
                    textContext.Done<User>(matches[0]);
                }
                else
                {
                    //multiple matches...give choice
                    PromptDialog.Choice(textContext, async (IDialogContext choiceContext, IAwaitable<User> choiceResult) =>
                    {
            ...
            }, "What user are you interested in (you can lookup by full/partial name or alias)?");
        }
    }
}